...
export function BrowserPane() {
  ...
  const reg: RegFn = (id) => (el) => { if (el) els.current.set(id, el); else els.current.delete(id); };
  ...
  useLayoutEffect(() => {
    ...
    if (browser.cursorTarget) {
      ...
      if (el) { const r = local(el); setCursor({ x: r.x + r.w / 2, y: r.y + r.h / 2 }); }
    } else setCursor(null);
    ...
    if (browser.clickPulse) {
      ...
      if (el) { const r = local(el); setRipple({ x: r.x + r.w / 2, y: r.y + r.h / 2, key: browser.clickPulse.key }); }
    }
  ...
  return (
    <div className="flex h-full flex-col bg-[#0B0D13]" data-testid="live-browser">
      ...
      <div className="flex h-9 shrink-0 items-center gap-1.5 border-b border-white/8 bg-[#12151E] px-2">
        <button data-testid="browser-back" className="rounded p-1.5 text-muted-foreground hover:bg-white/5 hover:text-foreground"><ArrowLeft className="size-3.5" /></button>
        <button data-testid="browser-forward" className="rounded p-1.5 text-muted-foreground/40"><ArrowRight className="size-3.5" /></button>
