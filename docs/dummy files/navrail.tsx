...
export function NavRail({ view, go, onOpenWorkspaces }: { view: View; go: (v: View) => void; onOpenWorkspaces: () => void }) {
  return (
    <nav data-testid="nav-rail" className="flex w-14 shrink-0 flex-col items-center gap-1 border-r border-white/8 bg-[#0E1119] py-2">
      <button
...
function Tip({ label }: { label: string }) {
  return (
    <span className="pointer-events-none absolute left-full z-50 ml-3 origin-left scale-90 whitespace-nowrap rounded-md border border-white/10 bg-popover px-2 py-1 text-[10px] text-foreground opacity-0 shadow-xl shadow-black/50 transition-all group-hover:scale-100 group-hover:opacity-100">
      {label}
    </span>
  );
}
