...
import { ChevronRight, X, AlertTriangle, Info, CircleX, Globe } from "lucide-react";
...
export function InspectorPane({ selected, onClose }: { selected: TimelineEvent | null; onClose: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col bg-[#0E1119]" data-testid="inspector-panel">
      ...
      {selected && (
        <div className="flex shrink-0 items-center gap-2 border-b border-cyan-400/20 bg-cyan-400/5 px-3 py-2" data-testid="inspector-event-banner">
          <Globe className="size-3 shrink-0 text-cyan-300" />
          <div className="min-w-0 flex-1">
            <p className="truncate text-[10px] font-medium text-foreground">{selected.label}</p>
            <p className="truncate font-mono text-[9px] text-muted-foreground">{selected.detail}</p>
          </div>
          <button data-testid="inspector-event-close" onClick={onClose} className="rounded p-0.5 text-muted-foreground hover:text-foreground">
            <X className="size-3" />
          </button>
        </div>
      )}
        ...
        {tab === "network" && (
          <div className="p-1.5" data-testid="inspector-network-list">
            ...
            <div className="grid grid-cols-[44px_1fr_40px_44px] gap-2 border-b border-white/8 px-2 py-1 font-mono text-[8.5px] uppercase tracking-wider text-muted-foreground">
              <span>Method</span><span>URL</span><span>Status</span><span className="text-right">Time</span>
            </div>
            {network.map((n) => (
          ...
          <div className="space-y-px p-1.5" data-testid="inspector-console-list">
            ...
            {logs.map((l) => (
              <div key={l.id} className="flex items-start gap-2 rounded px-2 py-1.5 font-mono text-[10px] hover:bg-white/[0.03]">
                {l.level === "error" ? <CircleX className="mt-0.5 size-3 shrink-0 text-red-400" /> : l.level === "warn" ? <AlertTriangle className="mt-0.5 size-3 shrink-0 text-amber-400" /> : <Info className="mt-0.5 size-3 shrink-0 text-muted-foreground" />}
