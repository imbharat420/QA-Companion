...
/**
 * THE TARGET SITE UNDER TEST — "Blixen Tours".
 *
 * This is the only component in the app that deliberately ignores our design
 * tokens. It simulates a third-party website inside the browser pane, so it has
 * to look like someone else's product: its own light palette, its own type,
 * its own hardcoded colours. Restyling it to our tokens would defeat the point.
 *
 * `reg(id)` hands each interactive element's DOM node back to the browser pane,
 * which is how the laser cursor, target box and typing overlay find real
 * on-screen coordinates instead of guessing.
 */
import { CheckCircle2, Mountain, ShieldCheck, Star } from "lucide-react";
import type { PageId } from "@/lib/api/types";
