...
import { ChevronDown, Command, Bell, Cpu, FolderGit2, Check, Home } from "lucide-react";
...
function ProjectSwitcher() {
  ...
  const { currentWorkspace, setCurrentWorkspace, workspaces } = useAgent();
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div data-testid="project-switcher-menu" className="absolute left-0 top-full z-50 mt-1 w-72 rounded-lg border border-white/10 bg-[#1A1E2B] p-1 shadow-2xl shadow-black/60">
            {workspaces.map((w) => (
              <button
                key={w.id}
                ...
                onClick={() => {
                  ...
                  setOpen(false);
                  toast.success(`Workspace switched → ${w.name}`);
                }}
                className="flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left transition-colors hover:bg-white/5"
              >
                <span>
                  <span className="block font-mono text-[11px] text-foreground font-semibold">{w.name}</span>
                  <span className="block text-[10px] text-muted-foreground truncate max-w-56">{w.path}</span>
                </span>
                {w.id === currentWorkspace.id && <Check className="size-3.5 text-cyan-300" />}
              </button>
            ))}
          </div>
        </>
...
export function TitleBar({ onOpenPalette, onGoHome }: TitleBarProps) {
  const { status, tokens, sessionId, currentWorkspace } = useAgent();
  ...
  return (
    <header data-testid="titlebar" className="flex h-9 shrink-0 items-center gap-3 border-b border-white/8 bg-[#0E1119] px-3 select-none">
      ...
      {onGoHome && (
        <button
          onClick={onGoHome}
          title="Return to All Projects Home"
          className="flex items-center gap-1 rounded-md border border-white/10 px-2 py-0.5 text-[10px] font-medium text-cyan-300 hover:bg-white/5"
        >
      ...
      <span className="rounded-md border border-white/10 px-2 py-0.5 font-mono text-[10px] text-muted-foreground">
      ...
      <div className="flex size-5.5 items-center justify-center rounded-full bg-blue-600 font-mono text-[10px] font-bold text-white">
        B
      </div>
    </header>
  );
