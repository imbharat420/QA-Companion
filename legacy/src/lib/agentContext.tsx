...
import { WORKSPACES, type Workspace } from "./mockData";
...
export function AgentProvider({ children }: { children: ReactNode }) {
  ...
  const [messages, setMessages] = useState<ChatMsg[]>([
    { id: 0, from: "agent", ts: now(), text: `Workspace ${WORKSPACES[0].name} loaded — .claude context, ${WORKSPACES[0].tests} tests, Chromium ready. Ask me to test, explore or fix anything.` },
  ]);
  ...
  const addWorkspace = (w: Workspace) => {
    setWorkspaces((prev) => [w, ...prev.filter((item) => item.id !== w.id)]);
  };
  ...
  const tl = (kind: TimelineEvent["kind"], label: string, detail: string, eventStatus: TimelineEvent["status"] = "ok") =>
    setTimeline((t) => [...t, { id: nid(), ts: now(), kind, label, detail, status: eventStatus }]);

  const net = (method: string, url: string, netStatus: number, ms: number) =>
    setNetwork((n) => [...n, { id: nid(), method, url, status: netStatus, ms }]);
  ...
  const startTicker = () => {
    const id = window.setInterval(() => setTokens((t) => t + 12 + Math.floor(Math.random() * 38)), 650);
    timers.current.push(id);
  };
  ...
  const runTask = useCallback((prompt: string) => {
    ...
    schedule(1200, () => msg("agent", `On it — opening ${currentWorkspace.name} and walking the booking flow end to end. Watch the browser.`));
    schedule(1900, () => {
      setStatus("executing");
    ...
    schedule(16000, () => {
      ...
      setApproval({
        ...
        desc: `The agent wants to click “Pay €348.00” on ${currentWorkspace.name} using card •• 4242.`,
